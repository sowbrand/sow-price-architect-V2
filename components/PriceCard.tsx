...
export const PriceCard: React.FC<PriceCardProps> = ({ result, productName, category, taxRegime }) => (
    <div className="bg-white rounded-xl border border-sow-border shadow-md relative overflow-hidden group flex flex-col h-auto min-h-[140px] p-6 transition-all hover:shadow-lg">
        ...
        <div className="relative z-10 flex flex-col gap-6 w-full h-full">
            <div className="w-full">
                ...
                <div className="text-5xl font-mono font-bold text-sow-dark tracking-tighter break-words leading-tight w-full min-w-0">
                    {result ? formatCurrency(result.suggestedSalePrice) : '---'}
                </div>
            </div>
            <div className="flex flex-wrap gap-6 border-t border-sow-border pt-4 w-full mt-auto">
